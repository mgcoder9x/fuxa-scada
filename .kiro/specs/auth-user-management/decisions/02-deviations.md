...
### DV-012: In-place FUXA-core edit — suppress `console.error` for expected sign-in outcomes (401/429) in `auth.service.ts`

- Date: 2026-07-17
- Phase: Post-flip commercial console hygiene (user item #2, explicitly approved: "sau đó thực hiện 2")
- Status: Active (CONFIRMED — browser-verified; user-directed)
- Links: D-003 (adapter-only boundary + no in-place FUXA-core edits), DV-011 (the prior sibling in-place FUXA-core client edit), N-062/N-077 (root-trace of the `[ERROR] Re` to this exact line), `client/src/app/_services/auth.service.ts`.
- Context: N-077's in-browser root-trace proved the `[ERROR] Re @ main.js` the user repeatedly flagged is FUXA-core `auth.service.ts` `signIn()` line 77 doing `console.error(err)` on EVERY signin failure — including the expected, fully-handled 401 (`invalid_credentials`) and 429 (`too_many_attempts`). The module client (`auth-management/**`) contains ZERO `console.error` (grep-verified), so this is purely FUXA-core behaviour surfacing through the Option-2 login seam (N-065).
- Statement (the deviation): I edited `auth.service.ts` in place — the `signIn` error callback now logs only when the failure is NOT an expected auth status: `if (!err || (err.status !== 401 && err.status !== 429)) { console.error(err); }`. `observer.error(err)` is unchanged, so the caller still receives the error and the UI still shows the alert. This is an in-place FUXA-core edit, which D-003 normally routes away from (same class as DV-011).
