...
- [ ] 1. Module scaffolding, data models, and layer interfaces
  - [ ] 1.1 Create the module skeleton, data models, and service/store/audit interfaces
...
- [ ] 2. Serialization module and FUXA-backed Store layer (§06, §11)
  - [ ] 2.1 Implement the shared serialization module with resilient parse
  ...
  - [ ]* 2.2 Write property test for metadata serialize→deserialize identity
...
- [ ] 3. Password_Hasher and the bcrypt seam (§03)
  - [ ] 3.1 Implement the `BcryptHasherAdapter` (Hash seam)
...
- [ ] 5. Token_Service and the JWT seam (§02)
  - [ ] 5.1 Implement the `TokenAdapter` (Token seam)
...
- [ ] 6. Brute-force protection guard (§10)
  - [ ] 6.1 Implement the per-username brute-force guard
    - Create `services/brute-force.js`: pure in-memory state machine `{failCount, lockedUntil}` keyed by submitted username with an **injected clock**; `checkAllowed`/`recordFailure`/`reset`; threshold/lockout/window config; `threshold=0` ⇒ fail-closed (always 429); lazy eviction of stale entries
    - _Requirements: 15.1, 15.2, 15.3, 15.4, 15.5_
  - [ ]* 6.2 Write property test for the lockout lifecycle
    - **Property 12: Lockout lifecycle matches the reference state machine**
    - **Validates: Requirements 15.1, 15.2, 15.3, 15.4, 15.5** — (P-012; owner §10); model-based, generator domain includes `threshold=0` and multiple usernames (isolation); min 100 iters
  - [ ]* 6.3 Write unit/edge tests for the guard
    - Below-threshold allowed, Nth failure locks + retryAfterMs, threshold-zero blocks two distinct users, success reset, lockout elapse recovery, per-username isolation, eviction shrinks the map
    - _Requirements: 15.1, 15.2, 15.3, 15.4, 15.5_
...
- [ ] 7. Authentication_Service — sign-in and sign-out (§01)
  ...
  - [ ]* 7.2 Write unit tests for sign-in outcomes and delegation
    - success→session; unknown→no token + recordFailure; bad_password→no token + recordFailure; missing_field validated before store/guard; assert single `verify(submitted, storedHash)` call and no direct equality on password
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5_
  - [ ]* 7.3 Write an integration test for the sign-in wiring
    - Seeded user signs in → 200 token that `Token_Service.verify` accepts; wrong password → 401 no token; unknown → 404 no token (real service→store/hasher/token, no mocks)
...
- [ ] 9. User_Service — CRUD (§04)
  - [ ] 9.1 Implement `User_Service`
    ...
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 6.1, 6.2, 6.3, 6.4, 7.1, 7.2, 7.3, 7.4, 7.5, 8.1, 8.2, 8.3, 8.5_
  - [ ]* 9.2 Write unit tests for CRUD outcomes
    - Create duplicate/missing-field; list excludes hash; get empty for missing; update retain-hash-on-omit and no-mutation-on-failure and missing→error; delete missing→error, cache eviction, and **last-admin refused with no mutation** (AC-8.5)
    - _Requirements: 5.1, 5.2, 5.3, 6.1, 6.2, 6.4, 7.3, 7.4, 7.5, 8.3, 8.5_

- [ ] 10. Checkpoint — service layer complete
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Audit_Logger and emission wiring (§09)
  - [ ] 11.1 Implement `Audit_Logger` and the FUXA winston `Audit_Sink`
    - Create `services/audit-logger.js`: `record(event)` void/total/non-throwing; shape check; emit one `AUDIT `-marked JSON line at `info` via an `Audit_Sink` default that calls FUXA's `runtime/logger` (no core edit); no ambient reads; caller-supplied ISO `timestamp`
...
- [ ] 12. Administrator bootstrap, rotation, and migration (§12)
  - [ ] 12.1 Implement the startup bootstrap routine
    - Create `services/bootstrap.js` `runBootstrap(deps)`: content-based empty-admin check via `isAdministrator` (§05); when none, seed **exactly one** admin with a CSPRNG one-time secret (never `'123456'`, cost 12), `groups=-1`, `metadata.mustRotate=true`, hash written verbatim via the adapter; disclose the secret once out-of-band; emit `bootstrap.seed` audit (AC-17.5); idempotent + retain-existing on non-empty (AC-17.4)
  ...
  - [ ]* 12.6 Write unit/integration tests for bootstrap and migration
    - Seed-once + idempotent restart (AC-17.1/17.4); `bootstrap.seed` audited (AC-17.5); seeded/legacy `'123456'` yields no usable authority pre-rotation and the module seed never verifies `'123456'`; migration flips `mustRotate` on a detected known-default admin
...
- [ ] 13. API layer — routers, authorization middleware, and mount
  - [ ] 13.1 Implement the authorization middleware seam
    - Create `api/authorization.middleware.js`: build `Identity` from `Token_Service.verify` (+ live `mustRotate` from the store), call `Authorization_Service.isAllowed`, short-circuit 401/403, never touch the store directly; fail fast if the service is unavailable
  ...
  - [ ] 13.5 Implement the composition root and the single FUXA mount line
    - Create `server/auth-management/index.js`: instantiate adapters → services (inject Password_Hasher, Token_Service, brute-force, Audit_Logger, stores), run `runBootstrap` at startup, and export the mounted router; add the **one** router-mount line in the appropriate FUXA API bootstrap file (the only permitted core touch, D-003)
...
- [ ] 15. Client session plumbing reuse and HTTP clients (§07, §08)
  - [ ] 15.1 Reuse and extend the client session plumbing
  ...
  - [ ] 15.2 Implement `AuthSignInClient`
    - Create the module-owned Angular service issuing `POST /api/signin` via `EndPointApi.getURL()`, resolving `{ token, username, fullname, roles }`, normalizing errors to stable ids (`invalid_credentials`/`user_not_found`/`too_many_attempts`/`missing_field`/`unexpected_error`); uses `Skip-Error` so 401 does not trigger global sign-out
...
- All 12 confirmed properties have a dedicated fast-check task (min 100 iterations, tagged): P-001 (3.3), P-002 (3.4), P-003 (2.5), P-004 (2.6), P-005 (2.2), P-006 (8.3), P-007 (5.3), P-008 (5.4), P-009 (12.4), P-010 (12.5), P-011 (8.4), P-012 (6.2).
...
```json
{
  "waves": [
    ...
    { "id": 3, "tasks": ["2.5", "2.6", "2.7", "3.3", "3.4", "3.5", "5.3", "5.4", "5.5", "8.1", "8.2", "15.4"] },
    { "id": 4, "tasks": ["7.1", "8.3", "8.4", "8.5", "9.1", "13.1"] },
    { "id": 5, "tasks": ["7.2", "7.3", "9.2", "12.1", "12.2", "13.2", "13.4"] },
    { "id": 6, "tasks": ["11.2", "12.3", "12.4", "12.5", "13.3"] },
    { "id": 7, "tasks": ["11.3", "12.6", "13.5", "16.1", "17.1"] },
    { "id": 8, "tasks": ["13.6", "16.2", "17.2", "17.3"] },
    { "id": 9, "tasks": ["17.4"] },
    { "id": 10, "tasks": ["17.5"] }
  ]
}
