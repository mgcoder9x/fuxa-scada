...
- [ ] 20. Runtime auth-configuration (D-049, design/13-runtime-config.md)
  ...
  - [x] 20.2 Client: `/auth/settings` runtime-configuration page
    - Standalone page (DV-010 presenter + jest) over a thin `AuthConfigClient`; renders the effective config, submits a bounded patch of ONLY changed keys, validates against SERVER-served bounds, surfaces per-field messages, two-step reset; gated by `settings.read` / `settings.manage` (grantable via the server catalog since task 23) + a permission-gated Settings tab
    - _Requirements: 16.1, 16.3_
    - _DONE 2026-07-27 (N-096): server now also serves its validation `bounds` (additive) so the client never hand-copies policy limits — the N-091 L3 lesson; client jest 123/10 suites (+18 page specs +3 regressions), server 211 (+1 bounds assertion), prod build 0, 34 i18n keys × 13 locales. LIVE: hints rendered FROM server bounds ("Allowed: 10–15"), out-of-range blocked locally with no request, and the FULL loop verified — change the minimum 12→16 in the UI ⇒ `POST /api/users` with a 13-char password is refused with `detailCode:password_too_short, params:{min:16}` ⇒ two-step reset in the UI returns it to 12. `operator1` (no `settings.read`): tab absent, page `Unauthorized!`, no form. **A browser-only defect was found and root-fixed in the process**: `[(ngModel)]` on `type="number"` yields a NUMBER (or `null` when cleared) while the form model assumed `string` → `TypeError: .trim is not a function` on every change-detection pass; the model now types fields `string | number` and every read goes through ONE `raw()` coercion boundary (3 regression specs)._
  - [x] 20.3 Advanced token-signing configuration (iss / aud / alg)
    ...
    - _DONE 2026-07-28 (D-054 Option B, N-099/N-100/N-101). The design-validation (N-099, verified against the installed jsonwebtoken) showed the §12 overlap window would force hand-rolling iss/aud validation inside `verify` for the primary unset→set case; the user chose **Option B** instead: expose iss/aud/alg at runtime but keep `verify` strict + library-enforced, so a change is a CONFIRMED, session-ending event (existing tokens then fail verify ⇒ re-login). SERVER: `AuthConfigService` validate/apply/bounds (alg HS-only per N-099, iss/aud null-or-bounded-string), `verify` UNCHANGED; server 216 (+5). CLIENT: presenter form/validate/buildPatch + a confirm-before-apply gate + "Advanced — Token signing" section (HS-only select fed from server bounds) + 9 i18n × 13 locales + 9 presenter specs; jest 136, prod build 0. LIVE e2e: alg HS256→HS384 → confirm "signs out ALL users" → PUT 200 "saved and applied" → the in-flight token then 401s on the next GET (session-ending) → Reset to defaults restored HS256. GOTCHA logged (N-101): restart the server after server-code edits — a stale in-memory module first rejected the valid patch as "unknown field"._
